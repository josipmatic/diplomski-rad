 ...
 * @return string
 */
function bojler_site_get_image_src( $image_id, $size = 'full' ) {
	if ( ! empty( $image_id ) ) {
 ...
 * @return string
 */
function bojler_site_get_background_image( $image_id = 0, $size = 'full' ) {
	if ( empty( $image_id ) ) {
		$image_id = get_post_thumbnail_id();
	}

	$src = bojler_site_get_image_src( $image_id, $size );
 ...
 * @return void
 */
function bojler_site_the_background_image( $image_id = 0, $size = 'full' ) {
	echo esc_attr( bojler_site_get_background_image( $image_id, $size ) );
}
 ...
 * @return string
 */
function bojler_site_get_asset_src( $filename = '' ) {
	if ( empty( $filename ) ) {
		return '';
	}
...
/**
 * Prints `bojler_site_get_asset_src` result
 *
 * @param string $filename Asset's filename.
 */
function bojler_site_the_asset_src( $filename = '' ) {
	echo esc_url( bojler_site_get_asset_src( $filename ) );
}
 ...
 * @return null|mixed
 */
function bojler_site_keep_resize_aspect_ratio( $output, $orig_w, $orig_h, $dest_w, $dest_h, $crop ) {
	if ( false !== $crop ) {
		return null;
	}
...
add_filter( 'image_resize_dimensions', 'bojler_site_keep_resize_aspect_ratio', 10, 6 );
